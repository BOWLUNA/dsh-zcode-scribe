...
| The `parameters` authoring DSL **rejects `required: false`** — optionality is the *absence* of the key. The rejection is a `defineTool()`-time throw, i.e. a boot failure | `UNSUPPORTED_SCHEMA: parameters.topic.required must be true when present`; asserted by `test/recall.test.mjs` › `declares an optional topic parameter` (`node test/run.mjs`) |
| `defineTool` wraps `isConcurrencySafe` as `(args) => validate(args).length > 0 ? false : user(args)`, and the host calls it with the real arguments (`dsh-tools/lib/index.js:2955`) | reading the wrapper's own source, then tests; asserted by `test/recall.test.mjs` › `declares itself concurrency safe, because it only reads` |
| `ctx.tools.register` validates only `output.schema` and stores the definition verbatim — skipping `defineTool` ships the authoring DSL to the provider, which then rejects the whole function schema | the sibling plugin `dsh-zcode-git`'s inline note, re-confirmed here by `test/recall.test.mjs` › `declares the full output contract the host validates` |
...
| `agent.session.header.cwd` is how a tool learns the session workspace | read in this repo at `index.js:170` (`exec?.agent?.session?.header?.cwd`). The sibling plugin `dsh-zcode-git` learns it the same way in its own `src/exec.js` (`resolveWorkdir(path, exec)`), which is a neighbour, not a root — `tools/verify-zcode-citations.mjs` resolves only this repo, the ZCode mirror and the DSH harness, so it is described here rather than cited |
